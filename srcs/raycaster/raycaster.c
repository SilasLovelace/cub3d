...
/*   Updated: 2025/02/18 13:47:04 by sopperma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void find_distance_and_line_length(t_ray *ray)
{
	...
	ray->line_height = mem->mlx_data->resolution_y / (ray->distance * cos(ray->angle - mem->player_pos->angle));
	...
	while (ray->y_iterator < mem->mlx_data->resolution_y)
	{
		...
		else
		{
			...
			unsigned int color = *(unsigned int*)(tex.addr + (tex_y * tex.line_length + ray->tex_x * (tex.bpp / 8)));
				my_mlx_pixel_put(ray->ray_num, ray->y_iterator, color);
		}
		ray->y_iterator++;
	} 
}
